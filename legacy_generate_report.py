...
def percentage_str(value: float) -> str:
    ...
    return babel.numbers.format_percent(
        round(value, precision), locale=settings.locale, decimal_quantization=False
    )
...
def currency_tick_prefix() -> str:
    if (
        babel.numbers.Locale.parse(settings.locale)
...
def calculate_value_change(
    values: pd.Series, iscurrency: bool = False, ispercentage: bool = False
) -> html.ValueChange:
...
def autofill(input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    start_date = input_data.loc[input_data.index[0], "date"] - datetime.timedelta(
        days=7
    )
    ...
    if "currency" in ticker.fast_info:
        if ticker.fast_info["currency"] != settings.currency:
            ...
            currency_ticker = yf.Ticker(
                f"{ticker.fast_info['currency']}{settings.currency}=X"
            )
            currency_rate = currency_ticker.history(start=start_date, interval="1d")
            yfdata[settings.autofill_price_mark] *= currency_rate[
                settings.autofill_price_mark
            ]
            yfdata["Dividends"] *= currency_rate[settings.autofill_price_mark]
    else:
        print_warning(
            "Ticker currency info is missing. "
    ...
    yfdata = yfdata.reset_index().rename(
        columns={"index": "date", "Date": "date", settings.autofill_price_mark: "price"}
    )
    ...
    if "return_tax" not in data.columns:
        data["return_tax"] = 0.0
    data["return_tax"] = (
        pd.to_numeric(data["return_tax"]).interpolate(method="pad").fillna(0.0)
    )
    ...
    for p in [
        ...
        "group",
        "info",
    ]:  # TODO: move this part out of the method
    ...
    if "return" not in data.columns:
        data["return"] = (
            data["amount"] * data["Dividends"].fillna(0.0) * (1 - data["return_tax"])
        )
...
def process_data(input_data, discard_zero_values: bool = True) -> pd.DataFrame:
    ...
    if (
        ("investment" in data.columns)
        and ("amount" in data.columns)
        and ("price" not in data.columns)
        and ("value" not in data.columns)
    ):
        data = autofill(data)
    ...
    data["net_investment"] = np.where(
        data["net_investment"] < 0, 0, data["net_investment"]
    )
...
def calculate_monthly_values(input: pd.DataFrame) -> pd.DataFrame:
    ...
    monthly = group_by_month.agg(
        {
            "date": "last",
            "value": "last",
            "net_investment": "last",
            "net_investment_max": "last",
            "return": "sum",
            "profit": "last",
            "relative_profit": "last",
            "period": "last",
            "sold": "last",
        }
    ).reset_index()

    monthly["date"] = monthly["date"].map(
        lambda x: x.replace(day=1, hour=0, minute=0, second=0)
    )
...
def fraction_string(input: pd.DataFrame) -> pd.Series:
    data = input.copy()
    data = data.assign(
        percentage_in_profitability="", of_profitability="", separator="", of_group=""
    )

    data["percentage_in_profitability"] = np.where(
        data["fraction_in_profitability"] < 1,
    ...
    if data["profitability"].nunique() > 1:  # if values are separated by profitability
        data["of_profitability"] = np.where(
            data["fraction_in_profitability"] < 1, " of " + data["profitability"], ""
        )
        data["separator"] = "<br>"
    else:
        data["separator"] = " / "
    data["separator"] = np.where(data["fraction_in_group"] < 1, data["separator"], "")
    data["of_group"] = np.where(
        data["fraction_in_group"] < 1,
    ...
    data.loc[
        data["fraction_in_profitability"] == data["fraction_in_group"],
        ["separator", "of_group"],
    ] = [["", ""]]

    return (
        data["percentage_in_profitability"]
        + data["of_profitability"]
        + data["separator"]
        + data["of_group"]
    )
...
def plot_sunburst(input: pd.DataFrame, values: str, label_text: str):
    ...
    for t in range(len(tree)):
        ...
        if values == "relative_profit":
            d[["profit", "net_investment_max"]] = (
                data.groupby(tree[: t + 1])
                .sum()
                .reset_index()[["profit", "net_investment_max"]]
            )
    ...
    return go.Sunburst(
        labels=graph_data["label"],
        ids=graph_data["id"],
        parents=graph_data["parent"],
        values=graph_data["value"],
        sort=False,
        customdata=graph_data["display_string"],
        marker=dict(colors=graph_data["color"]),
        branchvalues="total",
...
def plot_historical_data(
    dataframe: pd.DataFrame, values: str, label_text: str
) -> go.Figure:
...
def plot_historical_return(dataframe: pd.DataFrame, label_text: str) -> go.Figure:
    value_by_date = dataframe.pivot(index="date", columns="id", values="return").fillna(
        0.0
    )
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    customdata=np.transpose(
                        [str_value_by_date[a], str_value_by_date_cumulative[a]]
                    ),
                    marker=dict(color=asset_properties.loc[a, "color"]),
                    hovertemplate=(
                        f"%{{x|%B %Y}}<br>"
    ...
    fig.update_layout(
        yaxis2=dict(
            ...
            ticksuffix=currency_tick_suffix(),
            tickprefix=currency_tick_prefix(),
            side="right",
            overlaying="y",
            range=[0, max(value_by_date_sum.values) * 1.05],
            fixedrange=True,
        ),
        legend=dict(x=1.1),
    )
...
def plot_historical_relative_profit(dataframe: pd.DataFrame) -> go.Figure:
    value_by_date = dataframe.pivot(
        index="date", columns="id", values="relative_profit"
    )
    value_by_date = value_by_date.drop(
        value_by_date.columns[value_by_date.max() == -1], axis=1
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=overall.index,
            y=overall["values"] * 100,
            mode="lines+markers",
            name="Total",
            marker=dict(color=cyan),
            customdata=overall["strings"],
            hovertemplate=(
                f"%{{x|%B %Y}}<br>"
...
def plot_historical_asset_data(input: pd.DataFrame) -> go.Figure:
    ...
    if contains_non_zero_values(data["amount"]):
        data["str_net_investment"] += "<br><b>Amount:</b> " + data["amount"].apply(
            decimal_str
        )
    ...
    for p in input["period"].dropna().unique():
        ...
        pdata["red_fill"] = np.where(
            ...
            pdata["net_investment"],
            pdata["return_received"],
        )
        fig.add_trace(
            go.Scatter(
                x=pdata["date"],
    ...
    fig.update_layout(
        hoverlabel=dict(
            bgcolor=theme_colors["tab_background_color"],
    ...
    if (frequency in ["H", "BH"]) and (  # hourly data
        latest_date - latest_entry_date < datetime.timedelta(days=3)
    ):
        span = pd.DateOffset(days=5)
    elif (frequency in ["H", "BH", "B", "C", "D"]) and (  # daily data
        latest_date - latest_entry_date < datetime.timedelta(days=14)
    ):
...
def plot_yearly_asset_data(data: pd.DataFrame) -> go.Figure:
    ...
    yearly_data["relative_value_change"] = abs(
        yearly_data["value_change"] / yearly_data["net_investment_max"]
    )
    yearly_data["relative_return_received"] = (
        yearly_data["total_return_received"] / yearly_data["net_investment_max"]
    )
    ...
    yearly_data["str_total_return_received"] = (
        yearly_data["total_return_received"].apply(currency_str)
        + " / "
        + yearly_data["relative_return_received"].apply(percentage_str)
    )
    yearly_data["str_value_change_positive"] = (
        "+"
        + yearly_data["value_change_positive"].apply(currency_str)
        + " / "
        + yearly_data["relative_value_change"].apply(percentage_str)
    )
...
def configure_historical_dataview(
...
def get_overall_figures(statistic: str, label_text: str) -> html.Columns:
    ...
    if all(current_stats[statistic] == 0):
        return html.Columns(
            [
                html.Column(
                    content=historical.to_html(full_html=False, include_plotlyjs=True)
                )
            ]
        )
    ...
    return html.Columns(
        [
            html.Column(
                width=30,
                content=sunburst.to_html(full_html=False, include_plotlyjs=True),
            ),
            html.Column(
                content=historical.to_html(full_html=False, include_plotlyjs=True)
            ),
...
def create_asset_data_view(input: pd.DataFrame) -> str:
    ...
    if last_row["return_received"] != 0:
        ...
        statistics.append(
            html.Label(
                "Return received",
                html.Value(currency_str(last_row["return_received"]), valuechange=c),
            )
        )
...
def append_overall_data_tabs(document: html.Document):
    ...
    if assets["value"].any() > 0:
        ...
        change = calculate_value_change(
            total.set_index("date")["value_change"], iscurrency=True
        )
        label = html.Label(
            "Value", html.Value(currency_str(last_row["value"]), valuechange=change)
        )
        content = get_overall_figures("value", "Value")
        tabs.append(html.Tab(label, content, checked=True))

    change = calculate_value_change(
        total.set_index("date")["net_investment"], iscurrency=True
    )
    label = html.Label(
        "Funds invested",
        html.Value(currency_str(last_row["net_investment"]), valuechange=change),
    )
    ...
    if assets["return_received"].any() > 0:
        ...
        if last_row["return_received"] != 0:
            value = html.Value(
                currency_str(last_row["return_received"]), valuechange=change
            )
    ...
    label = html.Label(
        "Net profit",
        html.Value(currency_str(last_row["profit"]), valuechange=change).color(),
    )
    content = get_overall_figures("profit", "Net profit")
    tabs.append(html.Tab(label, content))

    change = calculate_value_change(
        total.set_index("date")["relative_profit"], ispercentage=True
    )
    label = html.Label(
        "Relative net profit",
        html.Value(
            percentage_str(last_row["relative_profit"]), valuechange=change
        ).color(),
    )
...
def calculate_total_historical_data(
    input: pd.DataFrame, name: str = "Total"
) -> pd.DataFrame:
    data = input.copy()
    ...
    group_total[
        ["id", "name", "symbol", "group", "account", "color", "comment", "info"]
    ] = np.nan
    for a in group_assets:
        asset_data = process_data(
            data[data["id"] == a].set_index("date").reindex(all_dates).reset_index(),
...
def append_asset_data_tabs(document: html.Document):
    ...
    for g in groups:
        ...
        group_accounts = group_accounts.agg(
            {"value": "sum", "active": "any"}
        ).reset_index()
        group_accounts.sort_values(
            by=["value", "active"], inplace=True, ascending=False
        )
        ...
        if (len(group_accounts) > 1) or (
            ...
            and (group_accounts[0] == " ")
            and (group_data["name"].nunique() > 1)
        ):
        ...
        for acc in group_accounts:
            ...
            account_assets.sort_values(
                by=["value", "active"], inplace=True, ascending=False
            )
            account_assets = account_assets["name"].unique()

            if (len(account_assets) > 1) and (acc != " "):
                account_total = calculate_total_historical_data(
                    account_data, f"{acc} Total"
                )
                content += create_asset_data_view(account_total)
                content += html.Divider()

            for a in account_assets:
                content += create_asset_data_view(
                    account_data[account_data["name"] == a]
                )
                if a != account_assets[-1]:  # add a divider, unless it's the last asset
                    content += html.Divider()

            if (
                acc != group_accounts[-1]
            ):  # add a double divider, unless it's the last account
            ...
            if entry.is_file() and entry.path.endswith((".yaml", ".yml")):
                ...
                if ("data" not in input) and (any([s in input for s in self.settings])):
                    ...
                    if settings_found == SettingsFound.YES:
                        reppiort.warn(
                            "Multiple settings files detected, expect trouble"
                        )
                        settings_found = (
...
if __name__ == "__main__":
    ...
    for s in settings:  # all settings are possible arguments
        parser.add_argument(
            f"--{s}",
            type=type(getattr(settings, s)),
            help=settings.get_description(s),
            choices=settings.get_allowed(s),
        )
    ...
    for entry in os.scandir(arguments.input_dir):
        if entry.is_file() and (
            entry.path.endswith(".yaml") or entry.path.endswith(".yml")
        ):
        ...
        if entry.is_file() and (
            ...
            if not any([s in input for s in settings]):
                try:
                    ...
                    info = (
                        pd.DataFrame()
                    )  # create empty dataframe in case of an error in input
    ...
    assets["id"] = (
        assets["group"] + assets["account"] + assets["name"]
    )  # + assets['symbol']
    group_by_name = assets.groupby("id")[
        ["id", "name", "group", "account", "value"]
    ].tail(1)
    ...
    for _, group in group_by_name.groupby("group"):
        boundary_colors = [
            px.colors.qualitative.Set1[index],
            px.colors.qualitative.Pastel1[index],
        ]
        c = px.colors.n_colors(
            boundary_colors[0],
            boundary_colors[1],
            max(len(group.index), 4),
            colortype="rgb",
        )
        c = c[: len(group.index)]
        group["color"] = c
    ...
    for i in current_stats["id"]:
        if contains_non_zero_values(assets.loc[i == assets["id"], "value"]):
            ...
            if current_stats.loc[i == current_stats["id"], "date"].iloc[0] < (
                latest_date
                - pd.tseries.frequencies.to_offset(settings.relevance_period)
            ):
                current_stats = current_stats[i != current_stats["id"]]
    ...
    report.append(
        f"<h3>Data from {date_str(earliest_date)} to {date_str(latest_date)}</h3>"
    )
    report.append(
        html.Button(
    ...
    report.append(
        f'<p class="footer">Report generated on {date_str(datetime.date.today())}, '
